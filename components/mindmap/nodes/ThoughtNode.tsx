...
export default function ThoughtNode({ data, selected }: NodeProps<MindMapNodeData>) {
  ...
  return (
    <div
      ...
      style={{
        width: 200,
        border: `2px dashed ${selected ? '#2dd4bf' : borderColor}`,
        boxShadow: selected
          ? `4px 4px 0 #2dd4bf`
    ...
    >
      ...
      <div className="flex items-center gap-2 px-2 py-1.5 border-b border-[#e2e8f0]/20">
        <span className="text-sm">💭</span>
        <span
          className="text-[9px] text-[#e2e8f0] truncate"
      ...
      <div className="px-2 py-1.5">
        <p className="text-xs text-white font-medium truncate leading-tight italic">
          {data.title || '(untitled)'}
        </p>
        {preview && (
          <p className="text-[10px] text-slate-500 truncate mt-0.5 italic">{preview}</p>
        )}
      </div>
